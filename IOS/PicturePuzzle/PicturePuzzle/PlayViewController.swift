...
class PlayViewController: UIViewController
{
    ...
    func InitImgvLock(p_view: UIButton, p_level: Int) -> Void
    {
        ...
        l_frm.size.height = l_frm.size.width
        l_frm.origin.x = 0.5 * (p_view.frame.size.width - l_frm.size.width)
        l_frm.origin.y = 0.5 * (p_view.frame.size.height - l_frm.size.height)
        ...
        {
            l_imgview_lock.frame.origin.x = p_view.frame.size.width - l_imgview_lock.frame.size.width
            l_imgview_lock.frame.origin.y = p_view.frame.size.height - l_imgview_lock.frame.size.height
            l_imgview_lock.alpha = 1
    ...
    func SetupView() -> Void
    {
        ...
        l_btn_coin_frm.origin.x = m_view_header.frame.size.width - l_btn_coin_frm.size.width - m_btn_back.frame.origin.x
        ...
        m_circle_time.trackColor = UIColor.init(white: 0.3, alpha: 1)
        m_circle_time.setColors(UIColor.whiteColor(), UIColor.redColor())
    ...
    func ShowGameEnd(p_iswin: Bool)
    {
        ...
        if IS_IPAD || IS_IPHONE_4_OR_LESS
        {
            self.m_view_tiles_photo.frame.origin.y = 0.5 * self.m_view_tiles_photo.frame.origin.x
        }
        ...
        var l_frm = CGRectMake(0, 0, 0, 0)
        l_frm.size.width = WidthForText("12345678912345", p_font: l_font, p_heigh: 10000)
        l_frm.size.height = HeightForText("12345678912345", p_font: l_font, p_width: l_frm.size.width)
        ...
        if IS_IPAD || IS_IPHONE_4_OR_LESS
        {
            l_frm.origin.y =  m_view_tiles_photo.frame.origin.y + m_view_tiles_photo.frame.size.height + 0.25 * m_array_tiles[0].frame.size.height
        }
        
        let l_lbl_gamewin = UILabel(frame: l_frm)
        ...
        l_lbl_hintone.textColor = l_lbl_gamewin.textColor
        ...
        l_lbl_corrected.textColor = l_lbl_gamewin.textColor
        ...
        l_3.textColor = l_0.textColor
        l_3.font = l_lbl_hintone.font
        l_3.text = String(l_count) + "/" + String(NUMBER_H_ITEM * NUMBER_V_ITEM)
...
//            })
        ...
        for i in 0..<m_array_tiles.count
        {
            ...
            if l_find != nil
            {
                if m_array_tiles[i].tag == l_find
                {
                    ...
                    self.m_array_tiles[l_find].layer.borderColor = UIColor.blackColor().CGColor
                }
    ...
    func XClick(sender: UIButton)
    {
        ...
        else
        {
            ...
            let l_btn_coin  = UIButton(frame: l_btn_coin_frm)
            l_btn_coin.setImage(UIImage(named: "btn_openphoto"), forState: .Normal)
            l_btn_coin.tag = sender.tag
            l_btn_coin.addTarget(self, action: #selector(PlayViewController.OpenPhoto(_:)), forControlEvents: .TouchUpInside)
            
            sender.addSubview(l_btn_coin)
            sender.alpha = 1
            l_btn_coin.Shake()
        }
    }
    ...
    {
        if PPCore.ShareInstance.m_coin < COIN_GAME_OVER
        {
            ShowToast("Your coins < \(COIN_GAME_OVER). Click bonus or buy the coin")
        }
        else
        {
            self.StartGame()
