...
-(void)CalculateView
{
    ...
    {
        frm.size.width = SCREEN_WIDTH - 6 *(SCREEN_WIDTH / 60.0);
    }
    ...
    if (IS_IPHONE_4_OR_LESS || IS_IPAD)
    {
        frm.origin.y = ViewHeader.frame.origin.y + ViewHeader.frame.size.height + 5;
    }
    else
    {
        frm.origin.y = ViewHeader.frame.origin.y + ViewHeader.frame.size.height + 15;
    }
    ...
    //if(IS_IPAD)
        l_fontbutton -= 5;
    //BtnHelp
    frm.size.width = 1.0/3 * (ViewButtons.frame.size.width - 5 * (SCREEN_WIDTH / 60.0));
    frm.size.height = ViewButtons.frame.size.height * 3.0/4;
    frm.origin.x = (SCREEN_WIDTH / 60.0);
    ...
    //GameCenter
    frm = _BtnRate.frame;
    frm.origin.x = ViewButtons.frame.size.width - frm.size.width - (SCREEN_WIDTH / 60.0);
