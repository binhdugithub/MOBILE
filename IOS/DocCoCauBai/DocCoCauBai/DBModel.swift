...
class DBModel
{
  ...
  {
    let l_dabPath = GetDocPathFile(p_name)
    //let l_dabPath = "/Volume/STUDY/" + p_name
    if !NSFileManager.defaultManager().fileExistsAtPath(l_dabPath)
    {
      ...
      {
        print("Creat database ok: \(l_dabPath)")
      }
  ...
  func CreateTable(p_table: String, p_database: String) -> Bool
  {
    
    let l_dabPath = GetDocPathFile(p_database)
    let l_funnyDB = FMDatabase(path: l_dabPath as String)
    
    if l_funnyDB == nil
    {
      print("Create table \(p_table) failed: \(l_funnyDB.lastErrorMessage())")
      return false
    }
    
    if l_funnyDB.open()
    {
      let sql_stmt = "CREATE TABLE IF NOT EXISTS \(p_table) (id INTEGER PRIMARY KEY NOT NULL, title CHAR (512) NOT NULL, content TEXT NOT NULL,imageurl CHAR(512), image BLOD DEFAULT(NULL),audiourl CHAR(256), audio BLOD DEFAULT (NULL), favorite BOOLEAN NOT NULL DEFAULT (0));"
      ...
      print("Create table ok at database: \(l_dabPath)")
      l_funnyDB.close()
      return true
    }
    else
